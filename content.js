    ...
    chrome.runtime.sendMessage({
      ...
      if (stats) {
        chrome.storage.local.get(['articles', 'links'], (result) => {
          ...
          // Fixed limits for compact sidebar
          const MAX_EXPLORE_NEXT = 5;
          const MAX_SUGGESTED = 3;
          const MAX_REFERENCED_IN = 3;

          const unreadLinks = allLinksOnPage
            .filter(link => !articles[link.url])
            .slice(0, MAX_EXPLORE_NEXT);
          ...
          // Get total articles count for stats
          const totalArticlesRead = Object.keys(articles).length;

          createSidebar(stats, visitCount, unreadLinks, incomingLinks, suggestedFromNetwork, totalArticlesRead, MAX_REFERENCED_IN);
        });
      }
    });
  }

  function createSidebar(stats, visitCount, unreadLinks, incomingLinks, suggestedFromNetwork, totalArticlesRead, maxReferencedIn) {
    ...
    const statsHTML = `
      <div class="wiki-sidebar-stats">
        <div class="wiki-stat-item">📚 ${totalArticlesRead} articles read</div>
      </div>
    `;

    let contextTrailHTML = '';
    if (incomingLinks.length > 0) {
      const linksList = incomingLinks.slice(0, maxReferencedIn).map(link => {
